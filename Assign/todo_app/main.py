...
def edit_todo(todo_id: int, request: Request, session: SessionDep):
    todo = session.get(Todo, todo_id)
    if not todo:
        raise HTTPException(status_code=404, detail="Todo not found")
...
async def edit_todo(todo_id: int, request: Request, session: SessionDep):
    content_type = request.headers.get("content-type", "")
    todo = session.get(Todo, todo_id)
    ...
    if "application/json" in content_type:
        data = await request.json()
        edit_todo_data = TodoEdit(**data)
    else:
        form = await request.form()
        edit_todo_data = TodoCreate(
            name=form["name"],
            description=form["description"],
            start_time=form["start_time"],
            end_time=form["end_time"]
    ...
    todo.name = edit_todo_data.name
    todo.description = edit_todo_data.description
    todo.start_time = edit_todo_data.start_time
    todo.end_time = edit_todo_data.end_time
